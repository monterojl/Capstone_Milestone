...
library(knitr)
opts_chunk$set(cache=TRUE, cache.path = 'Milestone_cache/')
library(quanteda); quanteda_options("threads"=3)
library(tm)
library(dplyr)
library(ggplot2)
library(gridExtra)
...
# Reading datasets
dataset.con <- file("Coursera-SwiftKey/final/en_US/en_US.twitter.txt", "r")
dataset.t<-readLines(dataset.con)
close(dataset.con)

size.t<-object.size(dataset.t)
words.t<-length(words(dataset.t))
lines.t<-length(dataset.t)

dataset.con <- file("Coursera-SwiftKey/final/en_US/en_US.blogs.txt", "r")
dataset.b<-readLines(dataset.con)
close(dataset.con)

size.b<-object.size(dataset.b)
words.b<-length(words(dataset.b))
lines.b<-length(dataset.b)

dataset.con <- file("Coursera-SwiftKey/final/en_US/en_US.news.txt", "r")
dataset.n<-readLines(dataset.con)
close(dataset.con)

size.n<-object.size(dataset.n)
words.n<-length(words(dataset.n))
lines.n<-length(dataset.n)
```
...
```{r datasubsetting, message = FALSE, warning = FALSE}
...
```{r dfmFrequencies, message = FALSE, warning = FALSE}
...
```{r pwords, message = FALSE, warning = FALSE, fig.width=6,fig.height=4}
...
```{r pies, message = FALSE, warning = FALSE, echo = FALSE}
# 1-grams
df <- data.frame(
   type = c("less than 3 times", "between 3 and 5 times", "more than 5 times"),
   number = c(dim(subset(ranking1, freq < 3))[1], dim(subset(ranking1, freq <= 5 & freq >=3))[1], dim(subset(ranking1, freq >5))[1]),
   freq = c(sum(subset(ranking1, freq < 3)$freq), sum(subset(ranking1, freq <= 5 & freq >=3)$freq), sum(subset(ranking1, freq > 5)$freq))
)


pie1<- ggplot(df, aes(x="", y=number, fill=type)) +
   geom_bar(width = 1, stat = "identity") + coord_polar("y", start=0) +
   labs(x="") + labs(y="tokens") + labs(title="1-grams")

pie2<- ggplot(df, aes(x="", y=freq, fill=type)) +
   geom_bar(width = 1, stat = "identity") + coord_polar("y", start=0) +
   labs(x="") + labs(y="frequency") + labs(title="1-grams by frequency")

# 2-grams
df <- data.frame(
   type = c("less than 3 times", "between 3 and 5 times", "more than 5 times"),
   number = c(dim(subset(ranking2, freq < 3))[1], dim(subset(ranking2, freq <= 5 & freq >=3))[1], dim(subset(ranking2, freq >5))[1]),
   freq = c(sum(subset(ranking2, freq < 3)$freq), sum(subset(ranking2, freq <= 5 & freq >=3)$freq), sum(subset(ranking2, freq > 5)$freq))
)


pie3<- ggplot(df, aes(x="", y=number, fill=type)) +
   geom_bar(width = 1, stat = "identity") + coord_polar("y", start=0) +
   labs(x="") + labs(y="tokens") + labs(title="2-grams")

pie4<- ggplot(df, aes(x="", y=freq, fill=type)) +
   geom_bar(width = 1, stat = "identity") + coord_polar("y", start=0) +
   labs(x="") + labs(y="frequency") + labs(title="2-grams by frequency")

# 3-grams
df <- data.frame(
   type = c("less than 3 times", "between 3 and 5 times", "more than 5 times"),
   number = c(dim(subset(ranking3, freq < 3))[1], dim(subset(ranking3, freq <= 5 & freq >=3))[1], dim(subset(ranking3, freq >5))[1]),
   freq = c(sum(subset(ranking3, freq < 3)$freq), sum(subset(ranking3, freq <= 5 & freq >=3)$freq), sum(subset(ranking3, freq > 5)$freq))
)


pie5<- ggplot(df, aes(x="", y=number, fill=type)) +
   geom_bar(width = 1, stat = "identity") + coord_polar("y", start=0) +
   labs(x="") + labs(y="tokens") + labs(title="3-grams")

pie6<- ggplot(df, aes(x="", y=freq, fill=type)) +
   geom_bar(width = 1, stat = "identity") + coord_polar("y", start=0) +
   labs(x="") + labs(y="frequency") + labs(title="3-grams by frequency")

# 4-grams
df <- data.frame(
   type = c("less than 3 times", "between 3 and 5 times", "more than 5 times"),
   number = c(dim(subset(ranking4, freq < 3))[1], dim(subset(ranking4, freq <= 5 & freq >=3))[1], dim(subset(ranking4, freq >5))[1]),
   freq = c(sum(subset(ranking4, freq < 3)$freq), sum(subset(ranking4, freq <= 5 & freq >=3)$freq), sum(subset(ranking4, freq > 5)$freq))
)


pie7<- ggplot(df, aes(x="", y=number, fill=type)) +
   geom_bar(width = 1, stat = "identity") + coord_polar("y", start=0) +
   labs(x="") + labs(y="tokens") + labs(title="4-grams")

pie8<- ggplot(df, aes(x="", y=freq, fill=type)) +
   geom_bar(width = 1, stat = "identity") + coord_polar("y", start=0) +
   labs(x="") + labs(y="frequency") + labs(title="4-grams by frequency")
```

Looking at the data gathered, we could see that when n is growing (n-grams), occurrences are less frequent. When looking at the single words (1-grams), most of the tokens appear few times (5 or less times), but less than 25% of the tokens cover 90% or more of the repetitions. So a few words are used very frequently, these are the ones known as stopwords (words used to connect in sentences). Nevertheless, those words haven't been excluded from the tokens generation since they are key in the 2, 3, 4, etc... grams to keep the consistency of the sentences and forecast the following word:
...
```{r pies plot1, message = FALSE, warning = FALSE, echo = FALSE, fig.width=10,fig.height=10}
grid.arrange(pie1, pie2, pie3, pie4, pie5, pie6, pie7, pie8, ncol=2, nrow=4)
```
